...
describe('@file picks', () => {
  ...
  it('resolves a nested directory through its source-owned value', async () => {
    const { source } = harness()
    await source.candidates(session('s1'), { query: 'client', position: 'inline', signal: new AbortController().signal })
    const outcome = source.onPick({
      candidate: { name: 'client', value: 'src/client', description: 'src' },
      session: session('s1'),
      position: 'inline',
      via: 'menu',
      span: { start: 0, end: 1, draftRev: 4 },
